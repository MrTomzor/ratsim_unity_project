...
- `maze/sectors/style`: (`rooms_and_corridors` only) `none` (default; sectors disabled), `quadrants`, or `orthogonal`. Both styles divide the maze into 4 sectors connected only via a central chamber at the world center, with intra-sector loops (`extra_corridor_fraction`) and no cross-sector loops — designed to break memoryless RL policies. `n_rooms` includes the chamber (e.g. `n_rooms=9` → 1 chamber + 8 sector rooms, ~2 per sector under round-robin). Memory_maze ignores this and emits a warning. Reference presets: `maze_sectored.yaml` (quadrants) and `maze_sectored_orthogonal.yaml`.
  ...
  - **`orthogonal`** — sectors are N / E / S / W spatial wedges with **no walls** between them. Rooms are rejection-sampled to lie entirely within their wedge with a buffer of `corridor_width/2 + 1` cells from the diagonal — wide enough that any L-corridor strip between two same-wedge rooms stays entirely in that wedge, so cross-sector floor merges can't happen. Boundary diagonal cells are just unclassified mask=true cells (not walls): corridors freely route through them when needed, so corridors look continuous. Per sector, one room is forced axis-aligned (centered on `mid_x` for N/S, `mid_z` for E/W) and a single straight cardinal corridor connects chamber to that "first room"; the rest of the sector branches via a per-sector MST that excludes the chamber. Result: exactly 4 chamber corridors, no cross-sector corridors, no walls splitting anything.
