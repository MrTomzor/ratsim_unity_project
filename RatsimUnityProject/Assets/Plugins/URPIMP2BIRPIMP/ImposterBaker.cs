...
public class ImposterBaker : MonoBehaviour
{
    ...
    void DrawMeshesToTarget(UnityShaderPass pass)
    {
        for (int i = 0; i < _renderers.Length; i++)
        {
            ...
            {
                int passIndex = -1;
                if (pass == UnityShaderPass.GBuffer)
                {
                    passIndex = _materials[i][j].FindPass("DEFERRED");
                    if (passIndex == -1)
                        passIndex = _materials[i][j].FindPass("Deferred");
                }
                else
                {
                    passIndex = _materials[i][j].FindPass(Enum.GetName(typeof(UnityShaderPass), pass));
                }
    ...
    void CaptureViews(RenderTexture albedoPackRT, RenderTexture normalPackRT)
    {
        ...
        // Render actual tiles
        {
            ...
            for (int frameIndex = 0; frameIndex < _snapshots.Length; frameIndex++)
            {
                ...
                gBuffer[0] = RenderTexture.GetTemporary(frameResolution, frameResolution, 0, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Linear);
                gBuffer[1] = RenderTexture.GetTemporary(frameResolution, frameResolution, 0, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Linear);
                gBuffer[2] = RenderTexture.GetTemporary(frameResolution, frameResolution, 0, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Linear);
                gBuffer[3] = RenderTexture.GetTemporary(frameResolution, frameResolution, 0, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Linear);

                RenderTexture depthBuffer = RenderTexture.GetTemporary(frameResolution, frameResolution, 32, RenderTextureFormat.Depth, RenderTextureReadWrite.Linear);
                ...
                // render gBuffers
                Graphics.SetRenderTarget(buffers, depthBuffer.depthBuffer);
                GL.Clear(true, true, Color.clear);
                ...
                // render dilateMask (alpha mask)
                {
                    ...
                    _imposterBakerMaterial.SetTexture("_AlphaMap", depthBuffer);
                    ...
                    Graphics.Blit(depthBuffer, dilateFrameRT, _imposterBakerMaterial, (int)ImposterBakerPass.AlphaCopy);

                    // send alpha to _albedoFrameRT
                    _imposterBakerMaterial.SetTexture("_AlphaMap", depthBuffer);
                    _imposterBakerMaterial.SetVector("_Channels", new Vector4(0, 0, 0, 1));
                    _imposterBakerMaterial.SetPass((int)ImposterBakerPass.AlphaCopy);
                    Graphics.Blit(depthBuffer, albedoFrameRT, _imposterBakerMaterial, (int)ImposterBakerPass.AlphaCopy);
                }

                // render depth
                {
                    _imposterBakerMaterial.SetTexture("_DepthMap", depthBuffer);
                    _imposterBakerMaterial.SetVector("_Channels", new Vector4(1, 0, 0, 0));
                    _imposterBakerMaterial.SetPass((int)ImposterBakerPass.DepthCopy);
                    Graphics.Blit(depthBuffer, depthFrameRT, _imposterBakerMaterial, (int)ImposterBakerPass.DepthCopy);
                }
    ...
    void CreateImposterAssets(RenderTexture albedoPackRT, RenderTexture normalPackRT)
    {
        ...
        // create material
        Shader imposterShader = Shader.Find("IMP/Imposter");
