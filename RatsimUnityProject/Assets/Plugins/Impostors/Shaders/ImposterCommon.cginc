...
void ImposterSample( in ImposterData imp, out half4 baseTex, out half4 worldNormal )//, out half depth )
{
    ...
    // Disable blending: Snap to the closest angle
    half maxW = max(weights.x, max(weights.y, weights.z));
    weights.x = (weights.x == maxW) ? 1.0 : 0.0;
    weights.y = (weights.y == maxW) ? 1.0 : 0.0;
    weights.z = (weights.z == maxW) ? 1.0 : 0.0;
    weights.xyz /= (weights.x + weights.y + weights.z); // Handle potential ties
